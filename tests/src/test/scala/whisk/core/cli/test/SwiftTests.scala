...
class SwiftTests
    ...
    it should "allow Swift actions to create a trigger" in withAssetCleaner(wskprops) {
        (wp, assetHelper) =>
            // create a trigger
            val triggerName = s"TestTrigger ${System.currentTimeMillis()}"
    ...
    it should "allow Swift actions to create a rule" in withAssetCleaner(wskprops) {
        (wp, assetHelper) =>
            ...
            assetHelper.withCleaner(wsk.action, "ActionThatCreatesTrigger") {
                (action, _) => action.create(name = "ActionThatCreatesTrigger", artifact = Some(createTriggerFile), kind = Some("swift:3"))
            }
            ...
            withActivation(wsk.activation, runCreateTrigger, initialWait = 5 seconds, totalWait = 60 seconds) {
                activation =>
                    // should be successful
                    activation.response.success shouldBe true
            }
            ...
            assetHelper.withCleaner(wsk.action, ruleActionName) {
                (action, _) => action.create(name = ruleActionName, artifact = Some(dummyFile), kind = Some("swift:3"))
            }

            // create an action that creates the rule
            val createRuleFile = TestUtils.getTestActionFilename("createRule.swift")
            assetHelper.withCleaner(wsk.action, "ActionThatCreatesRule") {
                (action, _) => action.create(name = "ActionThatCreatesRule", artifact = Some(createRuleFile), kind = Some("swift:3"))
            }

            // invoke the create rule action
            val runCreateRule = wsk.action.invoke("ActionThatCreatesRule", Map("triggerName" -> ruleTriggerName.toJson, "actionName" -> ruleActionName.toJson, "ruleName" -> ruleName.toJson))
            withActivation(wsk.activation, runCreateRule, initialWait = 5 seconds, totalWait = 60 seconds) {
                activation =>
                    // should be successful
                    activation.response.success shouldBe true
